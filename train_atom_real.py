...
def main():
    print("🚀 TRAINING ATOMUSDT (RANGE-BOUND) - REAL DATA - GPU")
    ...
    # Filter for ATOMUSDT
    atom_df = df[df['tic'] == 'ATOMUSDT'].copy().reset_index(drop=True)
    print(f"📊 ATOMUSDT records: {len(atom_df):,}")
    
    # Skip FeatureEngineer to avoid filtering issues - use raw data
    processed_df = atom_df.copy()
    ...
    # Save model
    model.save("crypto_atom_rangebound_gpu_model")
    print(f"💾 ATOMUSDT model saved in {training_time}")
