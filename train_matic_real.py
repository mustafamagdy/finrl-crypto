...
def main():
    print("🚀 TRAINING MATICUSDT - REAL DATA - GPU")
    
    # Load real data
    df = pd.read_csv('crypto_5currencies_2years.csv')
    df['date'] = pd.to_datetime(df['date'])
    
    # Filter for MATICUSDT
    matic_df = df[df['tic'] == 'MATICUSDT'].copy().reset_index(drop=True)
    print(f"📊 MATICUSDT records: {len(matic_df):,}")
    
    # Skip FeatureEngineer to avoid filtering issues - use raw data
    processed_df = matic_df.copy()
    ...
    model.learn(total_timesteps=80_000)  # Smaller dataset, fewer timesteps
    training_time = datetime.now() - start_time
    
    # Save model
    model.save("crypto_matic_real_gpu_model")
    print(f"💾 MATICUSDT model saved in {training_time}")
