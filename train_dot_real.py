...
def main():
    print("🚀 TRAINING DOTUSDT - REAL DATA - GPU")
    ...
    # Filter for DOTUSDT
    dot_df = df[df['tic'] == 'DOTUSDT'].copy().reset_index(drop=True)
    print(f"📊 DOTUSDT records: {len(dot_df):,}")
    
    # Skip FeatureEngineer to avoid filtering issues - use raw data
    processed_df = dot_df.copy()
    ...
    # Save model
    model.save("crypto_dot_real_gpu_model")
    print(f"💾 DOTUSDT model saved in {training_time}")
