...
def main():
    print("🚀 TRAINING LINKUSDT - REAL DATA - GPU")
    ...
    # Filter for LINKUSDT
    link_df = df[df['tic'] == 'LINKUSDT'].copy().reset_index(drop=True)
    print(f"📊 LINKUSDT records: {len(link_df):,}")
    
    # Skip FeatureEngineer to avoid filtering issues - use raw data
    processed_df = link_df.copy()
    ...
    # Save model
    model.save("crypto_link_real_gpu_model")
    print(f"💾 LINKUSDT model saved in {training_time}")
