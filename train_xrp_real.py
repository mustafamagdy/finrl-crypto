...
def main():
    print("🚀 TRAINING XRPUSDT (RANGE-BOUND) - REAL DATA - GPU")
    ...
    # Filter for XRPUSDT
    xrp_df = df[df['tic'] == 'XRPUSDT'].copy().reset_index(drop=True)
    print(f"📊 XRPUSDT records: {len(xrp_df):,}")
    
    # Skip FeatureEngineer to avoid filtering issues - use raw data
    processed_df = xrp_df.copy()
    ...
    # Save model
    model.save("crypto_xrp_rangebound_gpu_model")
    print(f"💾 XRPUSDT model saved in {training_time}")
