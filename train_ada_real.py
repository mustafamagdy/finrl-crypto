...
def main():
    print("🚀 TRAINING ADAUSDT - REAL DATA - GPU")
    ...
    # Filter for ADAUSDT
    ada_df = df[df['tic'] == 'ADAUSDT'].copy().reset_index(drop=True)
    print(f"📊 ADAUSDT records: {len(ada_df):,}")
    
    # Skip FeatureEngineer to avoid filtering issues - use raw data
    processed_df = ada_df.copy()
    ...
    # Save model
    model.save("crypto_ada_real_gpu_model")
    print(f"💾 ADAUSDT model saved in {training_time}")
